...
- (void)creatAlertController
{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"警告" message:@"当前网络不为WiFi，是否确定要继续连接" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)netDownLoad
{
    ...
    [mgr GET:@"http://api2.juheapi.com/jztk/query" parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [MBProgressHUD showError:@"网络异常，加载失败"];
            [view removeFromSuperview];
        });
